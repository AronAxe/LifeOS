...
```bash
bun "$HERMES_HOME/skills/webdesign/Tools/VerifyDesign.ts" "https://$DEPLOYED_URL" "$OUT/live-verify"
```
...
```bash
bun "$HERMES_HOME/skills/webdesign/Tools/VerifyDesign.ts" --a11y --lighthouse "https://$DEPLOYED_URL" "$OUT/live-quality"
```
...
```bash
# Preview
Use the installed `webdesign` skill's `Workflows/DeployDesign.md` with preview mode.
# Review preview URL
# Only then deploy to production
Use the installed `webdesign` skill's `Workflows/DeployDesign.md` with production mode only after explicit approval.
```
