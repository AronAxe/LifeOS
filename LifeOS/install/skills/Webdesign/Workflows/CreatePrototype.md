...
```bash
bun "$HERMES_HOME/skills/webdesign/Tools/DriveClaudeDesign.ts" prompt "$(cat /tmp/brief.md)"
```
...
mkdir -p "$OUT"
bun "$HERMES_HOME/skills/webdesign/Tools/DriveClaudeDesign.ts" screenshot "$OUT/v1.png"
```
...
```bash
bun "$HERMES_HOME/skills/webdesign/Tools/DriveClaudeDesign.ts" export bundle "$OUT"
```

### 7. Verify

```bash
bun "$HERMES_HOME/skills/webdesign/Tools/VerifyDesign.ts" "$OUT/index.html" "$OUT/verify"
```
...
```
Continue with the installed `webdesign` skill's `Workflows/IntegrateIntoApp.md`.
```
...
- One-line entry in the project's approved notes or changelog
