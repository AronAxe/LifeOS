...
> **Verification status:** the wrapper and its fail-closed notification boundary are covered offline. **Live-driving (boot-team/race/fleet/monitor) remains unproven in this port** because it requires a running macOS cmux instance and socket-auth handshake. To prove it, run the wrapper inside a cmux surface (inherited auth), or deliberately configure `CMUX_SOCKET_PASSWORD`. A socket password lets any local process holding it drive the fleet; never commit it.
...
bun $CT monitor --workspace workspace:1                  # poll; JSON reports transitions
...
**Integration boundary:** CMUX controls and observes terminal surfaces. It does not install a dashboard bridge, hooks, memory capture, or background service. During an interactive Hermes run, the agent may use `text_to_speech` for an approved spoken notification. For unattended CLI notification, set `CMUX_NOTIFY_ENDPOINT` to an explicit HTTP(S) adapter; without it, `voice` fails closed and `monitor` still emits JSON state.
...
- **Do not assume provider-specific hooks.** cmux may offer lifecycle integrations for particular agent CLIs, but HALOS does not install or configure them. The portable wrapper uses `surface-health` + `read-screen`; any hook-based adapter requires separate review, consent, and deduplication.
- **Sidebar metadata is cmux-owned state, not an automatic Hermes bridge.** `report_meta` / `report_meta_block` / `set-status` / `set-progress` / `log` may persist to cmux session JSON. Treat that schema as private and version-sensitive; do not ingest it into Hermes, memory, or a dashboard without a separately reviewed adapter and consent.
...
→ bun $CT monitor --workspace <ws>   # JSON transition output; optional configured notification
```
